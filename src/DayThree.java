    ...
    static long calculatePowerConsumption() {
        ...
        for (int i = 0; i < diagnosticReport.get(0).length(); i++) {
            ...
            if (countZeroes > countOnes) {
                gammaRateBitString.append(0);
            } else if (countZeroes < countOnes) {
                gammaRateBitString.append(1);
            } else {
                throw new IllegalStateException("Equal amount of ones and zeroes at index: " + i);
            }
        }


        for (int i = 0; i < gammaRateBitString.length(); i++) {
            if (gammaRateBitString.charAt(i) == '0') {
                epsilonRateBitString.append(1);
            } else if (gammaRateBitString.charAt(i) == '1') {
                epsilonRateBitString.append(0);
            } else {
                throw new IllegalStateException("gammaRateBitString must contain of 1 and 0 but was '" + gammaRateBitString.charAt(i) + "' at index: " + i);
            }
        }

        System.out.println("gammaRateBitString: " + gammaRateBitString);
        System.out.println("epsilonRateBitString: " + epsilonRateBitString);

        long gammaRate = Long.parseLong(gammaRateBitString.toString(), 2);
        long epsilonRate = Long.parseLong(epsilonRateBitString.toString(), 2);
